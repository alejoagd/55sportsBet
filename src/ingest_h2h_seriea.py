...
print("=" * 60)
print("🔍 VERIFICACIÓN DE ARCHIVOS - SERIE A")
print("=" * 60)
...
# ===============================
EXCEL_PATH_DEFAULT = str(PROJECT_ROOT / "data" / "H2H_SerieA.xlsx") 

# ⬅️ IMPORTANTE: ID de Serie A en tu base de datos
# SELECT id, name FROM leagues WHERE name LIKE '%Serie A%';
LEAGUE_ID_SERIE_A = 3  # ⬅️ AJUSTAR SEGÚN TU BASE DE DATOS
...
print(f"Liga: Serie A (ID: {LEAGUE_ID_SERIE_A})")
print("=" * 60)
print()
...
if DB_HOST in ['localhost', '127.0.0.1', '::1']:
    ...
    print()
    print("    PowerShell:")
    print("    $env:ENV_FILE='.env.production'; python src\\ingest\\ingest_h2h_seriea.py ingest")
    print()
    print("    CMD:")
    print("    set ENV_FILE=.env.production && python src\\ingest\\ingest_h2h_seriea.py ingest")
    print()
    print("    Linux/Mac:")
    print("    ENV_FILE=.env.production python src/ingest/ingest_h2h_seriea.py ingest")
    print()
...
print()
print("🚀 Continuando con la carga de datos de SERIE A...")
print()
...
def normalize_team_name(name: str) -> str:
    """Normalización de nombres de equipos de Serie A."""
    if name is None:
        return ""
...
def ensure_season(conn, year_start: int, year_end: int, league_id: int, seasons_map: Dict) -> int:
    ...
    start_date = f"{year_start}-08-16"  # Fecha típica inicio Serie A
    end_date = f"{year_end}-05-25"      # Fecha típica fin Serie A
...
@app.command()
def ingest(
    excel_path: str = typer.Option(EXCEL_PATH_DEFAULT, help="Ruta al Excel de Serie A"),
    league_id: int = typer.Option(LEAGUE_ID_SERIE_A, help="ID de Serie A en la BD")
):
    """
    Carga histórico de partidos de Serie A desde Excel H2H_SerieA.xlsx
    """
    
    print("=" * 70)
    print("🇮🇹 INGESTA DE DATOS HISTÓRICOS - SERIE A (ITALIA)")
    print("=" * 70)
    print()
    ...
    if not os.path.exists(excel_path):
        ...
        print()
        print("Ubica el archivo H2H_SerieA.xlsx en la carpeta 'data' o especifica:")
        print("  python src\\ingest\\ingest_h2h_seriea.py ingest --excel-path C:\\ruta\\completa\\archivo.xlsx")
        return
    
    print(f"📂 Archivo Excel: {excel_path}")
    print(f"🏆 Liga: Serie A (ID={league_id})")
    print()
